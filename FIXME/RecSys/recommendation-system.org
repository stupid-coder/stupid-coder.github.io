...
例如: 假设一个长度为 r 的用户 u 到用户 r 的有向路径.设 $f_{1},...,f_{r}$ 表示用户 u 到用户的有向路径上的线性变换函数.则用户 u 对物料 k 的预估行为值 $\hat{r_{uk}^{()}}$ 可以应用 r 个线性变换来获得:
\begin{equation}
  \hat{r_{uk}^{v}} = (f_{1} \cicr f_{2} \cicr ... \cicr f_{r})(r_{vk})
\end{equation}
