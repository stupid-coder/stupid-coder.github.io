...
矩阵 U 和 V 的每一列都是潜在向量,矩阵 U 和 V 的每一行为潜在系数.矩阵 U 的第 ith 行 $\vec{u}_{i}$ 对应这用户的因子,包含 k 个元素,为用户映射到 k 个潜在因子上的向量.同理,矩阵 V 的每一行为物料的潜在因子向量,在 k 维潜在因子表示的.

则行为矩阵中的得分可以用用户的潜在向量和物料的潜在向量的内积表示:
\begin{equation}
  r_{i,j} \approx \vec{u}_{i} \cdot \vec{v}_{j}
\end{equation}
